...
| Captures consistently return empty / "no on-screen window" | On Linux: first call `list_apps` and use the exact returned app name (`Google-chrome` is common; `Chrome` may not match). In scheduled/fresh sessions, launch through a wrapper that exports the interactive `DISPLAY`, `WAYLAND_DISPLAY`, `XDG_RUNTIME_DIR`, and `DBUS_SESSION_BUS_ADDRESS`, then verify with `list_apps` before capture. If still empty, run `hermes computer-use doctor`. On Windows: you may be in Session 0 (SSH session) instead of the interactive desktop — see the cua-driver `WINDOWS.md` deep-dive |

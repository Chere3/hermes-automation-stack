...
REQUIRED_SCOPES_PATH = HERMES_HOME / "google_required_scopes.json"
...
def _scopes_for_services(raw_services: str | None) -> list[str]:
    if not raw_services or raw_services.strip().lower() == "all":
        return list(SCOPES)
    ...
    if unknown:
        valid = ", ".join([*SERVICE_SCOPES, "all"])
        raise ValueError(f"Unknown services: {', '.join(unknown)}. Valid values: {valid}")
...
def _missing_scopes_from_payload(payload: dict) -> list[str]:
    ...
    if REQUIRED_SCOPES_PATH.exists():
        try:
            required_scopes = json.loads(REQUIRED_SCOPES_PATH.read_text(encoding="utf-8"))
        except Exception:
            required_scopes = SCOPES
    return sorted(scope for scope in required_scopes if scope not in granted)
...
def exchange_auth_code(code: str):
    ...
    requested_scopes = pending_auth.get("scopes", SCOPES)
    ...
    TOKEN_PATH.chmod(0o600)
    REQUIRED_SCOPES_PATH.write_text(json.dumps(requested_scopes, indent=2), encoding="utf-8")
    REQUIRED_SCOPES_PATH.chmod(0o600)
...
def revoke():
    ...
    PENDING_AUTH_PATH.unlink(missing_ok=True)
    REQUIRED_SCOPES_PATH.unlink(missing_ok=True)
...
def main():
    ...
    try:
        selected_scopes = _scopes_for_services(args.services)
